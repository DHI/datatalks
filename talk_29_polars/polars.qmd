...
Filtering on date/datetime is similar to any other filter:
...
#| output-location: slide
(danube
    .set_sorted("date") # we know the data is sorted, otherwise use .sort("date")
...
(danube
    ...
    .group_by_dynamic("date", every="1y")
    .agg(pl.col("discharge").max())
    .pipe(px.bar, x="date", y="discharge")
)
```
...
Re-do, but this time only for the spring months:
